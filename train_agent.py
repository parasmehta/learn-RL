...
from custom_env import CustomCartPoleEnv, create_custom_env
...
# 1 - Build an agent
# 1.1 - Get the default config of xxxConfig()
register_env("custom_cartpole", create_custom_env)
...
# (lr)
# 1.4 - Introduce the environment to the agent's config
config.environment(env="custom_cartpole", env_config={
...
# except that this time we have a trained agent
# 4.1 - Create an environment similar to the training env.
env = gym.make("custom_cartpole", render_mode="rgb_array")
s, _ = env.reset()
